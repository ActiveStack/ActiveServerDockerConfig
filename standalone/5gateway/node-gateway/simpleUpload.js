var http = require( 'http' ),
    ...
    emitProgress = 3,
    uploadThreshold = 1024 * 1024 * 1024,	// 1GB
    serialzedFieldThreshold = 10 * 1024 * 1024,
    maxFileSize = 1024 * 1024 * 1024,	// 1GB
    checkContentLength = false,
    removeIncompleteFiles = false,
    logging = 'debug:on,1:on,2:on,3:on,4:on,console:on,file:on,record:off',
    propFile = (process.argv[2] || "resources/env.properties"),
    dir =  '/tmp/';
	...
	getHtmlForm = function ( req, res, next ) {
        ...
        res.end( '<html><head>\
        		 ...
        		 <input type="hidden" name="testVar" value="myValue"/>\
    ...
    handleFormRequest = function ( req, res, next ) {
        var receivedFields = {},
            ...
            config = {
                ...
                holdFilesExtensions : holdFilesExtensions,
                ...
                requestTimeOut : requestTimeOut, // 5 secs
                ...
                resumeRequestOnError : resumeRequestOnError,
                ...
                emitFileProgress : emitFileProgress,
                ...
                emitProgress : emitProgress, // false, 3, 10, 100
                ...
                uploadThreshold : uploadThreshold , // bytes, default 1GB
                ...
                serialzedFieldThreshold : serialzedFieldThreshold,
               
                    // max bytes allowed for a single file
                maxFileSize : maxFileSize, // bytes, default 1GB
                ...
                removeIncompleteFiles : removeIncompleteFiles,
                    ...
                    // See the Readme!
                logging : logging, // <-- turn off 2nd level to see only warnings, and parser overall results
                ...
                listeners : {
                    ...
                    'progress' : function ( json ) {                              
                        log( '\n\033[1;32mProgress..' + (json.ratio * 100).toFixed(2) + '% [' + json.bytes + ' of ' + (json.bytes / json.ratio).toFixed(0) + ' bytes]\033[0m' );
                    },
                    'load' : function ( json ){
                    	log('LOAD EVENT');
                    	if (json.hasOwnProperty('name') && json.hasOwnProperty('value')) {
                    		if (json.name === 'testVar') {
                    			var testVar = json.value;
                    			console.log('Test Var === ' + testVar);
                    			var cerr = {};
                    			cerr.code = 'aborted';
                    			cerr.message = 'Invalid User';
                    			cerr.stack = '<STACK/>';
                    			console.log('Attempting to Close Connection.');
                    			this.closeConnection(cerr);
                    		} else {
...
prop.readProperties(propFile, function(props){
	dir = props['fileDirectory'] || dir;
	requestTimeOut = (props['requestTimeOut'] || requestTimeOut);
	emitProgress = (props['emitProgress'] || emitProgress);
	logging = (props['logging'] || logging);
	console.log('Using ' + dir + ' as file directory');
	console.log('Request Timeout: ' + requestTimeOut);
	console.log('Logging: ' + logging);
	console.log('Emit Progress: ' + emitProgress);
	server = connect( getHtmlForm , handleFormRequest );

	server.listen( props['fileserver.port'] );

	log();
	log( ' ->\033[1m started at: \033[32m' + new Date() + '\033[0m' );
	log( ' ->\033[1m listening on: \033[36mhttp://localhost:' + props['fileserver.port'] + '/\033[0m' );
